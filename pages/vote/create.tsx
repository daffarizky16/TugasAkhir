...
export default function NewVote() {
  ...
  const createVote = () => {
    //Validasi
    if (title === '') {
      showAlert({ title: 'Maaf', subtitle: 'Judul tidak boleh kosong' });
      return;
    }
    if (candidates.length < 2) {
      showAlert({ title: 'Maaf', subtitle: 'Minimal harus ada 2 kandidat' });
      return;
    }
    if (startDate > endDate) {
      showAlert({ title: 'Maaf', subtitle: 'Tanggal mulai tidak boleh lebih besar dari tanggal selesai' });
      return;
    }
    if (candidates.some((c) => c.name === '')) {
      showAlert({ title: 'Upss', subtitle: 'Nama Kandidat tidak boleh kosong' });
      return;
    }
  ...
  return (
    <div>
      ...
      <div className="container mx-auto py-10">
        <Image src={'/images/buatvote.png'} alt="vote" width={300} height={300} objectFit="contain" />
        ...
        <form className="flex flex-col">
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-col">
              ...
              <Form onChange={setTitle} value={title} placeholder="Contoh : Voting Calon Ketua BEM Periode 2023/2024" className="w-1/2" />
            </div>
            <div className="flex flex-col w-2/3">
              ...
              <div className="inline-flex">
                ...
                <ReactDatePicker locale={'id'} showTimeSelect dateFormat="Pp" selected={endDate} minDate={startDate} onChange={(date) => date && setEndDate(date)} className="w-full border bg-zinc-100 border-transparent py-2 px-3" />
              </div>
            </div>
